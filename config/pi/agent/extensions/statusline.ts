    ...
    ctx.ui.setFooter((tui: any, theme: any, footerData: any) => {
      ...
      return {
        ...
        render(width: number): string[] {
          const cwd = truncateToWidth(path.basename(ctx.cwd) || ctx.cwd, 24, "…");
          const branch = footerData.getGitBranch?.();
          const branchPart = branch ? ` ${truncateToWidth(branch, 28, "…")}` : "no-git";
          ...
          const extParts = statusParts(footerData.getExtensionStatuses?.() ?? new Map());

          const leftItems = ["π", cwd, branchPart];
          const rightItems = [modelPart, ctxPart, usagePart, ...extParts].filter(Boolean) as string[];
          ...
          const compactRight = [
            ...
            ctxPart ? theme.fg("warning", ctxPart) : undefined,
            theme.fg("success", usagePart),
          ].filter(Boolean).join(theme.fg("borderMuted", " │ "));
          const compactRightRaw = [modelPart, ctxPart, usagePart].filter(Boolean).join(" │ ");
          const compactGap = width - visibleWidth(left) - visibleWidth(compactRight);
          if (compactGap >= 1) return [left + " ".repeat(compactGap) + compactRight];

          return [truncateToWidth(`${leftItems.join(" ")} ${compactRightRaw}`, width, "…")];
        },
      };
    });
  }
